...
/** detects when a ParseNode is a variable and when two variables
 * are the same
 */
export interface MguVariableManager {
	isInternalNodeForVariable(parseNode1: ParseNode): boolean;
}
...
export class MguFinder {
	...
	*/
	...
	constructor(startingOrderedPairsOfNodes: OrderedPairOfNodes[], private variableManager: MguVariableManager) {
	...
	/** true iff parseNode2 contains a variable equal to parseNodeForVariable */
	private containsVariable(parseNode2: InternalNode, variable: string): boolean {
		let result: boolean = (this.variableManager.isInternalNodeForVariable(parseNode2) &&
			GrammarManager.getTokenValueFromInternalNode(parseNode2) == variable);
		let i = 0;
		while (!result && i < parseNode2.parseNodes.length) {
			const parseNode: ParseNode = parseNode2.parseNodes[i++];
			if (parseNode instanceof InternalNode)
				result = this.containsVariable(parseNode, variable);
		}
		return result;
	}

	//#region  applyRule5
	protected replaceNodeInDescendants(variableToBeReplaced: string, replacingNode: InternalNode,
		...
		for (let i = 0; i < nodeToBeReplaced.parseNodes.length; i++) {
			...
			if (this.variableManager.isInternalNodeForVariable(descendant) &&
				GrammarManager.getTokenValueFromInternalNode(<InternalNode>descendant) == variableToBeReplaced)
				nodeToBeReplaced.parseNodes.splice(i, 1, replacingNode);
			else if (descendant instanceof InternalNode)
				this.replaceNodeInDescendants(variableToBeReplaced, replacingNode, descendant);
		}
	}
	applyRule5(i: number, node1: InternalNode, node2: InternalNode) {
		const variableToBeReplaced = GrammarManager.getTokenValueFromInternalNode(node1);
		for (let j = 0; j < this.currentOrderedPairsOfNodes.length; j++) {
			if (j != i)
				this.replaceNodeInDescendants(variableToBeReplaced, node2, this.currentOrderedPairsOfNodes[j].parseNode1);
			this.replaceNodeInDescendants(variableToBeReplaced, node2, this.currentOrderedPairsOfNodes[j].parseNode2);
		}
		this.workingVarsForWhichRule5hasAlreadyBeenApplied.add(variableToBeReplaced);
	}
	...
	tryToPerformAnActionForCurrentPair(i: number): boolean {
		...
		const isParseNode1AVariable = this.variableManager.isInternalNodeForVariable(parseNode1);
		// const isParseNode2WorkingVar = GrammarManager.isInternalParseNodeForWorkingVar(parseNode2);
		const isParseNode2AVariable = this.variableManager.isInternalNodeForVariable(parseNode2);

		let hasBeenPerformedOneAction = false;

		if (!isParseNode1AVariable && !isParseNode2AVariable && parseNode1.label == parseNode2.label) {
			// rule 1
			this.applyRule1(i, parseNode1, parseNode2);
			hasBeenPerformedOneAction = true;
		} else if (!isParseNode1AVariable && !isParseNode2AVariable && parseNode1.label != parseNode2.label) {
			// rule 2
		...
		else if (isParseNode1AVariable && isParseNode2AVariable &&
			GrammarManager.areParseNodesEqual(parseNode1, parseNode2)) {
			// rule 3
			...
			hasBeenPerformedOneAction = true;
		} else if (isParseNode1AVariable &&
			this.workingVarsForWhichRule5hasAlreadyBeenApplied.has(GrammarManager.getTokenValueFromInternalNode(parseNode1)) &&
			!this.containsVariable(parseNode2, GrammarManager.getTokenValueFromInternalNode(parseNode1))) {
			//TODO1 you cannot just use the string, because 'ph' is not a variable, if on the right side of the pair
