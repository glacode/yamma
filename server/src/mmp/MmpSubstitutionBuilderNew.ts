...
import { OrderedPairOfNodes, UnificationAlgorithmState } from './WorkingVarsUnifierFinder';
import { MguFinder } from './MguFinder';
import { MguVariableManagerForLogicalVarsAndWorkingVars } from './MguVariableManagerForLogicalVarsAndWorkingVars';
...
export class MmpSubstitutionBuilderNew {
	...
	*/
	...
	private foundFormulaWithoutParseNode = false;
	...
	addPair(logicalSystemParseNode: InternalNode, mmpProofStepParseNode: InternalNode,
		orderedPairsOfNodes: OrderedPairOfNodes[]) {
		const orderedPairOfNodes: OrderedPairOfNodes = {
			parseNode1: logicalSystemParseNode,
			parseNode2: mmpProofStepParseNode
		};
		orderedPairsOfNodes.push(orderedPairOfNodes);
	}
	private tryToAddPair(mmpProofStep: MmpProofStep, logicalSystemParseNode: InternalNode,
		orderedPairsOfNodes: OrderedPairOfNodes[]) {
		if (mmpProofStep.formula != undefined && mmpProofStep.parseNode == undefined)
			this.foundFormulaWithoutParseNode = true;
		else if (mmpProofStep.parseNode != undefined)
			this.addPair(logicalSystemParseNode, mmpProofStep.parseNode, orderedPairsOfNodes);
	}
	buildStartingPairsForMGUAlgorithmForEHyps(): OrderedPairOfNodes[] {
		const orderedPairsOfNodes: OrderedPairOfNodes[] = [];
		let i = 0;
		while (!this.foundFormulaWithoutParseNode && i < this.logicalSystemEHyps.length) {
			if (i < this.eHypUSteps.length && this.eHypUSteps[i] !== undefined)
				// eHypUSteps[i] is an actual step
				this.tryToAddPair(this.eHypUSteps[i]!, this.logicalSystemEHyps[i].parseNode,
					orderedPairsOfNodes);
			i++;
		}
		return orderedPairsOfNodes;
	}
	buildStartingPairsForMGUAlgorithm(): OrderedPairOfNodes[] {
		const orderedPairOfNodes: OrderedPairOfNodes[] =
			this.buildStartingPairsForMGUAlgorithmForEHyps();
		if (!this.foundFormulaWithoutParseNode)
			this.tryToAddPair(this.uProofStep, this.assertion.parseNode, orderedPairOfNodes);
		return orderedPairOfNodes;
	}
	...
	buildSubstitutionForExistingParseNodes(): SubstitutionResult {
		...
		let substitutionResult: SubstitutionResult = {
			hasBeenFound: false,
			substitution: undefined
		};
		const startingPairsForMGUAlgorthm: OrderedPairOfNodes[] =
			this.buildStartingPairsForMGUAlgorithm();
		const mguVariableManager: MguVariableManagerForLogicalVarsAndWorkingVars = new
			MguVariableManagerForLogicalVarsAndWorkingVars(this.assertion.parseNode, this.outermostBlock.v);
		const mostGeneralUnifier: MguFinder =
			new MguFinder(startingPairsForMGUAlgorthm, mguVariableManager);
		mostGeneralUnifier.findMostGeneralUnifier();
		if (mostGeneralUnifier.currentState == UnificationAlgorithmState.complete) {
			// the MGU algorithm stopped found a valid substitution
			substitutionResult = {
				hasBeenFound: true,
				substitution: mostGeneralUnifier.mostGeneralUnifierResult
			};
		} //TODO1 add else and info for diagnostic if workingVarsUnifierFinder.currentState == UnificationAlgorithmState.clashFailure or
		// workingVarsUnifierFinder.currentState == UnificationAlgorithmState.occourCheckOrderedPair
	...
	buildSubstitution(): SubstitutionResult {
		let substitutionResult: SubstitutionResult = { hasBeenFound: false, substitution: undefined };
		if (this.eHypUSteps.length <= this.logicalSystemEHyps.length) {
			substitutionResult = this.buildSubstitutionForExistingParseNodes();
