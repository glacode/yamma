...
test('Test file inclusion', () => {
	...
	expect(tokens[1].filePath).toContain(path.normalize('server/src/__test__/../mmTestFiles/included1.mm'));
	...
	expect(tokens[6].filePath).toContain(path.normalize('server/src/__test__/../mmTestFiles/include.mm'));
	...
	tokens.forEach((mmToken: MmToken) => {
		if (mmToken.value === 'mpd.2') {
			...
			containsMpd2 = true;
			expect(mmToken.filePath).toContain(path.normalize('server/src/__test__/../mmTestFiles/included3.mm'));
